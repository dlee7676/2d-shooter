...
// events
	...
	if (leveltime == 200) {
		makeEnemy(0, -10, 0, boss, -1, 350, 200, 0, 0, 25000, 6);
	}
}
	...
	if ((*pEnemies)[i].getLife() < 4000) {
		if ((*pEnemies)[i].getCooldown() <= 0) {
			...
			(*pEnemies)[i].fire(enemyBullets, D3DXVECTOR3(newTarget.x+(*pEnemies)[i].getPos(0)+10, newTarget.y+(*pEnemies)[i].getPos(1)+10,0), 
				D3DXVECTOR3((*pEnemies)[i].getPos(0)+10,(*pEnemies)[i].getPos(1)+10,0), MAX_BULLETS, blueBall, 3, 3);
		...
		if ((*pEnemies)[i].getCooldown() <= 0) {
			newTarget = (*pEnemies)[i].rotateAim((*pEnemies)[i].getRotateDirection(), PI/12);
			(*pEnemies)[i].fire(enemyBullets, D3DXVECTOR3(newTarget.x+(*pEnemies)[i].getPos(0)+10, newTarget.y+(*pEnemies)[i].getPos(1)+10,0), 
				D3DXVECTOR3((*pEnemies)[i].getPos(0)+10,(*pEnemies)[i].getPos(1)+10,0), MAX_BULLETS, redBall, 2, 3);
			(*pEnemies)[i].setCooldown(1);
		}
		(*pEnemies)[i].setCooldown((*pEnemies)[i].getCooldown()-1);
	}
		...
		if ((*pEnemies)[i].getCooldown() <= 0) {
			for (int j = 0; j < 8; j++) {
				...
				(*pEnemies)[i].fire(enemyBullets,D3DXVECTOR3(shot.x+spiralPos.x+(*pEnemies)[i].getPos(0)+10, shot.y+spiralPos.y+(*pEnemies)[i].getPos(1)+10, 0), 
					D3DXVECTOR3(spiralPos.x+(*pEnemies)[i].getPos(0)+10, spiralPos.y+(*pEnemies)[i].getPos(1)+10, 0), MAX_BULLETS, blueBall, 4, 3);
			}
		...
		Enemy newStream;
		newStream.init((*pEnemies)[i].getPos().x,(*pEnemies)[i].getPos().y,0,boss,-1,0,0,0,0,1,1);
		...
		if ((*pEnemies)[i].getCooldown() == 0) {
			...
			(*pEnemies)[i].fire(enemyBullets, D3DXVECTOR3(newTarget.x+(*pEnemies)[i].getPos(0)+10, newTarget.y+(*pEnemies)[i].getPos(1)+10,0), 
				D3DXVECTOR3((*pEnemies)[i].getPos(0)+10,(*pEnemies)[i].getPos(1)+10,0), MAX_BULLETS, blueBall, 1, 3);
			for (int j = 0; j < subunits.size(); j++) {
				...
				if ((*pEnemies)[i].getRotateDirection() == 0) {
					...
					newTarget = subunits[j].rotateAim((*pEnemies)[i].getRotateDirection(), PI/16);
					subunits[j].fire(enemyBullets, D3DXVECTOR3(newTarget.x+subunits[j].getPos(0)+10, newTarget.y+subunits[j].getPos(1)+10,0), 
						D3DXVECTOR3(subunits[j].getPos(0)+10,subunits[j].getPos(1)+10,0), MAX_BULLETS, redBall, 1, 3);
				}
				else {
					(*pEnemies)[i].setRotateDirection(0);
					...
					subunits[j].fire(enemyBullets, D3DXVECTOR3(newTarget.x+subunits[j].getPos(0)+10, newTarget.y+(*pEnemies)[i].getPos(1)+10,0), 
						D3DXVECTOR3(subunits[j].getPos(0)+10,(*pEnemies)[i].getPos(1)+10,0), MAX_BULLETS, largeGreen, 1, 2);
				}
				(*pEnemies)[i].setCooldown(1);
			}
		}
